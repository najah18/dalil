...
(function($){
    ...
    redux.field_objects.wpestate_property_card_details_customizer   = redux.field_objects.wpestate_property_card_details_customizer || {};
    ...
    redux.field_objects.wpestate_property_card_details_customizer.init = function( selector ) {
        if ( !selector ) {
            selector = $( document ).find( ".redux-group-tab:visible" ).find( '.redux-container-media-card-details:visible' );
        }
        ...
        $( selector ).each(
            function() {
                ...
                el.find( '.remove-image, .remove-file' ).unbind( 'click' ).on(
                    'click', function() {
                        redux.field_objects.wpestate_property_card_details_customizer.removeFile( $( this ).parents( 'fieldset.redux-field:first' ) );
                    }
                );
                // Upload media button
                el.find( '.media_upload_button' ).unbind().on(
                    'click', function( event ) {
                        ...
                        var tracker= jQuery(this).attr('data-attr-button-no');
                        redux.field_objects.wpestate_property_card_details_customizer.addFile( event, $( this ).parents( 'fieldset.redux-field:first' ),tracker );
                    }
                );
            }
        );
    ...
    redux.field_objects.wpestate_property_card_details_customizer.addFile = function( event, selector ,tracker) {
        ...
        frame.on(
            'select', function() {
                ...
                selector.find( '#url_field_'+tracker ).val( attachment.attributes.url );
                selector.find( '#preview_field_'+tracker ).attr('src', attachment.attributes.url );
    ...
    redux.field_objects.wpestate_property_card_details_customizer.removeFile = function( selector ) {
