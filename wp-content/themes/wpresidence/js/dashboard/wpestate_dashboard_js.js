...
function wpestate_dash_add_new_plan(){
  jQuery('#add_new_floor_plan').click(function(event){
    ...
    var to_add_form='<div class="floor_plan_unit_wrapper row">'+
...
/**
 * Invoice Filter Handler
 * 
 * Handles the filtering of invoices via AJAX requests.
 * Manages form submission, displays results, and handles errors.
 * 
 * @since 1.0
 */
...
function wpestate_filter_invoices() {
    ...
    var start_date = jQuery('#invoice_start_date').val();
    var end_date = jQuery('#invoice_end_date').val();
    var type = jQuery('#invoice_type').val();
    var status = jQuery('#invoice_status').val();
    var nonce = jQuery('#wpestate_invoices_actions').val();
    
    jQuery.ajax({
        type: 'POST',
        url: wpestate_dashboard_js_vars.ajaxurl,
        dataType: 'json',
        data: {
        ...
        success: function(response) {
            if (response.success) {
                ...
                jQuery('.invoice_unit').remove();
                jQuery('.wpestate_dashboard_table_list_header ').after(response.data.results);


                jQuery('#invoice_confirmed').empty().append(response.data.invoice_confirmed);
            }
        },
...
/**
 * Handles adding comments to the CRM dashboard via AJAX.
 * Manages the comment form submission and updates the UI with the response.
 * 
 * @since 1.0.0
 */
function wpestate_crm_add_comment_dashboard() {
  const commentButton = jQuery('#crm_insert_comment');
  const commentTextarea = jQuery('#crm_new_commnet');
  const notesWrapper = jQuery('#show_notes_wrapper');
  ...
  commentButton.on('click', function(event) {
      ...
      const data = {
          ...
          item_id: jQuery(this).data('postid'),
          content: content,
          security: jQuery('#wpestate_crm_insert_note').val()
      };
      ...
      jQuery.ajax({
          ...
          success: function(response) {
              if (response.success) {
                  ...
                  notesWrapper.prepend(response.data.html);
                  
                  // Clear the textarea
                  commentTextarea.val('');
                  
                  // Optional: Show success message
                  // wpestate_show_notification('success', response.data.message);
              } else {
                  // Handle error response
                  const errorMessage = response.data.message || wpestate_dashboard_js_vars.strings.error_adding_comment;
                  // wpestate_show_notification('error', errorMessage);
                  console.error('Error adding comment:', errorMessage);
              }
          },
          error: function(jqXHR, textStatus, errorThrown) {
              // Handle AJAX error
              const errorMessage = wpestate_dashboard_js_vars.strings.ajax_error || 'Failed to add comment. Please try again.';
              // wpestate_show_notification('error', errorMessage);
  ...
  // Optional: Add keyboard shortcut (Ctrl/Cmd + Enter) to submit
  commentTextarea.on('keydown', function(event) {
